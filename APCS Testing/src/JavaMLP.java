/*
 * MLP neural network in Java by Phil Brierley www.philbrierley.com
 * 
 * This code may be freely used and modified at will
 * 
 * Tanh hidden neurons Linear output neuron
 * 
 * To include an input bias create an extra input in the training data and set
 * to 1
 * 
 * Routines included:
 * 
 * calcNet() WeightChangesHO() WeightChangesIH() initWeights() initData()
 * tanh(double x) displayResults() calcOverallError()
 * 
 * compiled and tested on Symantec Cafe Lite
 */
...
public class JavaMLP {
	...
	public static void displayResults() {
		for (int i = 0; i < numPatterns; i++) {
			patNum = i;
			calcNet();
			System.out.println("pat = " + (patNum + 1) + " actual = "
					+ trainOutput[patNum] + " neural model = " + outPred);
		}
	}
