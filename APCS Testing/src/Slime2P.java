...
label0:
    ...
    private void DrawSlimers()
    {
        ...
        int dcx = (p1X * nWidth) / 1000;
        int dcy = (p1Y * nHeight) / 1000;
        int dcr = 10;
        screen.fillOval(dcx - (dcr / 2), dcy - (dcr / 2), dcr / 2, dcr / 2);
        /////////////////
        ...
        if(nScore > 8)
        {
            ...
            {
                screen.setColor(Color.black);
                screen.drawArc(j, i1 + j5, l1, dvx, -30, -150);
            } while(++j5 < 3);
            return;
        }
        if(nScore < 2)
        {
            int dy = nWidth / 20;
            int dvy = nHeight / 20;
            int k = (p2X * nWidth) / 1000 - dy;
            int j1 = (7 * nHeight) / 10 - ((p2Y - 40) * nHeight) / 1000;
            int k5 = 0;
            do
            {
                screen.setColor(Color.black);
                screen.drawArc(k, j1 + k5, dy, dvy, -10, -150);
            } while(++k5 < 3);
        }
    }
    ...
    private void MoveBall()
    {
        ...
        if(!fEndGame)
        {
            ...
            // If ball is higher than slime and distance > 5 and distance < 125
            if(dy > 0 && distSqr < 15625 && distSqr > 25)
            {
                int dist = (int) Math.sqrt(distSqr);
                int j1   = ((dx * dvx) + (dy * dvy)) / dist;
                ballX    = p1X + (dx * 63) / dist;
                ballY    = p1Y + (dy * 125) / dist;
                
                if(j1 <= 0)
                {
                    ballVX += p1XV - (2 * dx * j1) / dist;
            ...
            {
                int dist = (int) Math.sqrt(distSqr);
                int k1   = ((dx * dvx) + (dy * dvy)) / dist;
                ballX    = p2X + (dx * 63) / dist;
                ballY    = p2Y + (dy * 125) / dist;
                
                if(k1 <= 0)
                {
                    ballVX += p2XV - (2 * dx * k1) / dist;
                    if(ballVX < -15)
                        ballVX = -15;
                    if(ballVX > 15)
                        ballVX = 15;
                    ballVY += p2YV - (2 * dy * k1) / dist;
                    if(ballVY < -22)
                        ballVY = -22;
                    if(ballVY > 22)
                        ballVY = 22;
