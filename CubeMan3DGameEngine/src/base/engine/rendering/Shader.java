...
import base.engine.entity.light.*;
import base.engine.rendering.resourceManagement.ShaderResource;
...
public class Shader {
	...
	public void updateUniforms(Transform transform, Material material, RenderingEngine renderingEngine) {
		...
		Matrix4f MVPMatrix   = renderingEngine.getMainCamera().getViewProjection().times(worldMatrix);
		...
		for (int i = 0; i < resource.getUniformNames().size(); i++) {
			...
			if (uniformType.equals("sampler2D")) {
				...
				if(uniformName.equals("C_eyePos"))
					setUniform(uniformName, renderingEngine.getMainCamera().getTransform().getTranslation());
				else
	...
	private HashMap<String, ArrayList<GLSLStruct>> findUniformStructs(String shaderText) {
		...
		while (structStartLocation != -1) {
			...
			while (componentSemicolonPos != -1
					&& componentSemicolonPos < braceEnd) {
