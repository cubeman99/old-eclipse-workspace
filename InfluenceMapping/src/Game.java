...
public class Game extends JComponent {
	...
	public Game() {
		...
		backgroundBuffer = new BufferedImage(VIEW_WIDTH, VIEW_HEIGHT, BufferedImage.TYPE_INT_RGB);
		hudBuffer		 = new BufferedImage(VIEW_WIDTH, VIEW_HEIGHT, BufferedImage.TYPE_INT_ARGB);
		
		try {
			ImageData.loadAllImages();
		} catch (IOException e) {
			e.printStackTrace();
		}
		
        installListeners();
        TeamControl.initializeTeams();
        
        world		= new World(GAME_WIDTH / 32, GAME_HEIGHT / 32);
	...
	public void initializeGame() {
		...
		//FileReader.readFile("C:/workspace/InfluenceMapping/testText.txt");
		//FileReader.readFile("res/testText.txt");
        //Image img = NewImageLoader.loadImage("images/map1.png");
		
        addEntity(world);
        addEntity(nodeMap);
        addEntity(unitControl);
	}
...
//				keyReleased[e.getKeyCode()] = true;
	...
	public static Vector getMouseVector() {
	...
	public void update() {
		...
		viewX = Game.unitControl.player.viewX - (VIEW_WIDTH / 2);
		viewY = Game.unitControl.player.viewY - (VIEW_HEIGHT / 2);
	...
	public static int getMouseTileX() {
		return (int) (mouseX / world.tileSize);
	}

	public static int getMouseTileY() {
		return (int) (mouseY / world.tileSize);
	}
	
	public static Graphics getHudGraphics() {
		return hudBuffer.getGraphics();
	}
	...
	public static Rect getViewRect() {
		...
		r.y = viewY;
		r.width  = VIEW_WIDTH;
		r.height = VIEW_HEIGHT;
		return r;
	}
	
	public void render() {
		//hudBuffer	= new BufferedImage(GAME_WIDTH, GAME_HEIGHT, BufferedImage.TYPE_INT_ARGB);
		Graphics2D hg2 = (Graphics2D) getHudGraphics();
		Graphics g = buffer.getGraphics();
		// clear the HUD bufer
		AlphaComposite ac = AlphaComposite.getInstance(AlphaComposite.DST_OUT);
		hg2.setComposite(ac);

		hg2.setColor(new Color(0, 0, 0, 255));
		hg2.fillRect(0, 0, VIEW_WIDTH, VIEW_HEIGHT);
		Graphics hg = getHudGraphics();
		
		// Draw a black background
		//g.setColor(Color.black);
		//g.fillRect(0, 0, GAME_WIDTH, GAME_HEIGHT);
		g.drawImage((Image) backgroundBuffer, 0, 0, null);
		// Draw All Entities
		for (int i = 0; i < entities.size(); i++) {
		...
		hg.drawString("FPS: " + fps, 16,  VIEW_HEIGHT - 32);
		hg.drawString("Entities: " + entities.size(), 16,  VIEW_HEIGHT - 50);
	...
	public void paintComponent(Graphics g) {
		...
		// Draw a black "Void" background
		g.setColor(Color.black);
		g.fillRect(0, 0, VIEW_WIDTH, VIEW_HEIGHT);
		
		// Draw View Image
		Rect r = getViewRect();
		g.drawImage(buffer, 0, 0, VIEW_WIDTH, VIEW_HEIGHT, (int)r.getX1(), (int)r.getY1(), (int)r.getX2(), (int)r.getY2(), null);
		//g.drawImage(buffer, 0, 0, null);
		
		// Draw the HUD
		g.drawImage(hudBuffer, 0, 0, null);
	}
