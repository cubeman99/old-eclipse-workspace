...
public class Game extends JComponent {
	...
	public static final int CELLSW      = 100;
	public static final int CELLSH      = 100;
	public static final int CELLSCALE   = 16;
	public static final boolean WRAP    = false;
	public static boolean running 		= false;
	public static int tickTimer         = 0;
	public static boolean[][] cells     = new boolean[CELLSW][CELLSH];
	public static boolean[][] cellsNext = new boolean[CELLSW][CELLSH];
...
//				keyReleased[e.getKeyCode()] = true;
	...
	public static int getCell(int x, int y) {
		if (!WRAP && (x < 0 || y < 0 || x >= CELLSW || y >= CELLSH)) {
			return 0;
		}
		while(x < 0)
			x += CELLSW;
	...
	public void update() {
		...
		if ((running || keys.tick.isPressed) && tickTimer == 0) {
			tickCells();
		}
		
		int msx = (int) Math.floor(mouseX / CELLSCALE);
		int msy = (int) Math.floor(mouseY / CELLSCALE);
		if (msx >= 0 && msy >= 0 && msx < CELLSW && msy < CELLSH) {
			if (mbLeft)
				cells[msx][msy] = true;
			if (mbRight)
				cells[msx][msy] = false;
			if (mbMiddlePressed)
				cells[msx][msy] = !cells[msx][msy];
		}
	...
	public void render() {
		...
		g.fillRect(0, 0, CELLSCALE * CELLSW, CELLSCALE * CELLSH);
		...
		for (int x = 0; x < CELLSW; x++) {
			for (int y = 0; y < CELLSH; y++) {
				if (cells[x][y]) {
					g.fillRect(x * CELLSCALE, y * CELLSCALE, CELLSCALE, CELLSCALE);
				}
			}
		}
		g.setColor(Color.lightGray);
		for (int x = 0; x < CELLSW; x++)
			g.drawLine(x * CELLSCALE, 0, x * CELLSCALE, CELLSH * CELLSCALE);
		for (int y = 0; y < CELLSH; y++)
			g.drawLine(0, y * CELLSCALE, CELLSW * CELLSCALE, y * CELLSCALE);
