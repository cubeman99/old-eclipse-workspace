...
public class GMath {
	...
	/** Return an array of points by a grid on a rectangle. **/
	public static Vector[] makeGridPoints(Rectangle rect, int amountX, int amountY, boolean offset) {
		Vector[] points = new Vector[amountX * amountY];
		int i = 0;
		for (int y = 0; y < amountY; y++) {
			for (int x = 0; x < amountX; x++) {
				if (offset) {
    				points[i++] = new Vector(
						rect.x1() + ((((double) x + 0.5) / (double) amountX) * rect.width()),
						rect.y1() + ((((double) y + 0.5) / (double) amountY) * rect.height())
					);
				}
				else {
    				points[i++] = new Vector(
    					rect.x1() + (((double) x / ((double) amountX - 1.0)) * rect.width()),
    					rect.y1() + (((double) y / ((double) amountY - 1.0)) * rect.height())
    				);
				}
			}
		}
