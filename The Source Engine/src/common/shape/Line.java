...
public class Line extends Shape {
	...
	/** Return if this line is touching a circle. **/
	public boolean touching(Circle c) {
		return (getClosestPoint(c.position).distanceTo(c.position) < c.radius);
	}
	
	/** Return all the intersection points with a circle. **/
	public Vector[] intersectWithCircle(Circle c) {
		Vector p1 = end1.minus(c.position);
		Vector p2 = end2.minus(c.position);

		double dx = p2.x - p1.x;
		double dy = p2.y - p1.y;
		double dr = GMath.distance(dx,  dy);
		double r  = c.radius;
		double D  = (p1.x * p2.y) - (p2.x * p1.y);

		double discriminant = (double) Math.sqrt(((r * r) * (dr * dr)) - (D * D));
		...
		if (discriminant == 0) {
			...
			return new Vector[] {new Vector(lx1 + c.position.x, ly1 + c.position.y)};
		}
		...
		double ly2 = ((-D * dx) - (GMath.abs(dy) * discriminant)) / (dr * dr);
		
		return new Vector[] {new Vector(lx1 + c.position.x, ly1 + c.position.y), new Vector(lx2 + c.position.x, ly2 + c.position.y)};
	}
	...
	@Override
	/** Compute and return the center point of the line. **/
	...
	/** Return the rectangular bounding box for this line. **/
	@Override
	...
	public Line transform(Transformation t) {
		t.apply(end1);
		t.apply(end2);
		return this;
	}
	...
	@Override
	/** Draw the filled line (no difference than normal). **/
